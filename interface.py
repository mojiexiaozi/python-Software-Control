...
class Interface(object):
    ...
    def load_window(self):
        self.design_window()
        self._window = Gui.Window(self.window_title, self._layouts.layout, alpha_channel=0.8)
...
class MainInterface(Interface):
    def design_window(self):
        ...
        controls.empty()
        controls.pack(Gui.Text('WelCome!', key='__MESSAGE__', size=[40, 1], font=("Helvetica", 18)))
        self.layouts.pack(controls)

        controls.empty()
        # controls.pack(Gui.T(' ' * 20))
        controls.pack(Gui.Button('record', key='__RECORD__', font=("Helvetica", 18), size=(12, 1)))
        controls.pack(Gui.Button('playback', key="__PLAYBACK__", font=("Helvetica", 18), size=(12, 1)))
        controls.pack(Gui.Button('review', key="__REVIEW__", font=("Helvetica", 18), size=(12, 1)))
...
class RecordingInterface(Interface):
    def design_window(self):
        ...
        controls.pack(Gui.Button('quit', disabled=False, key="__QUIT__", font=("Helvetica", 18), size=(12, 1)))
        self.layouts.pack(controls)
...
class ReviewInterface(Interface):
    def design_window(self):
        ...
        controls.empty()
        button_size = (5, 1)
        controls.pack(Gui.Button("Load", key="__LOAD_EVENTS__", size=button_size))
